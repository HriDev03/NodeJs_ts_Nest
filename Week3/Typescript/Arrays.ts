...
const nums1:number[] = new Array(1,2,3,4,5,6)
const names:string[] = Array.of("Hridyesh","Sharma","Dev")
...
const numToStr: string[] =numbers.map((currVal:number)=>{
...
const numToStrr: number[] =numbers.filter((currVal:number)=>
    currVal > 3
)
console.log(numToStrr);

const evenNums:number[]=numbers.filter((curVal:number)=>{
    return curVal%2==0
})
...
const names3:string[] = names2.filter((currVal:string)=>{
    return currVal.startsWith('A');
})
console.log(names3);

const lenGreater15:string[]=names2.filter(name=>name.length>15);
